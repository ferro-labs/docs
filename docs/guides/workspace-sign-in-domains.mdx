...
Ferro Labs Managed will create a pending verification record and show you the DNS TXT record that must be added before the domain becomes active.
...
Ferro Labs Managed checks the TXT record on `_ferrocloud-challenge.<domain>`. It also accepts the same token on the root domain if your DNS provider has limited host support.
...
- Ferro Labs Managed resolves the domain to the workspace.
